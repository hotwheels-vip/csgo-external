...
namespace rn::detail
{
	...
	public:
		...
		matrix_t( matrix_t&& rhs ) noexcept
		{
			*this = std::move( rhs );
		}

		explicit matrix_t( const type_array& values ) : _data( values ) { }

		template< typename... arguments >
		matrix_t( arguments&&... args ) : _data{ static_cast< float >( std::forward< arguments >( args ) )... }
		{
		}
		...
		matrix_t& operator=( matrix_t&& rhs ) noexcept
		{
			_data = rhs._data;
		...
		{
			return ( num_cols == 1 || num_rows == 1 ) && ( num_cols == 1 ? num_rows : num_cols ) <= 4;
		}
		...
		matrix_t& operator+=( const float rhs )
		{
			for ( std::size_t i = 0; i < size( ); ++i ) {
				at( i ) += rhs;
			}
		...
		matrix_t operator+( const float rhs ) const
		{
			auto lhs = *this;
			lhs += rhs;
			return lhs;
		...
		bool operator==( const matrix_t& rhs )
		{
			for ( std::size_t i = 0; i < size( ); ++i ) {
				if ( at( i ) != rhs( i ) ) {
					return false;
		...
		float& operator( )( const std::size_t row_index, const std::size_t col_index )
		{
		...
		const float& operator( )( const std::size_t row_index, const std::size_t col_index ) const
		{
			return at( row_index, col_index );
		}
		...
		float& at( const std::size_t row_index, const std::size_t col_index )
		{
		...
		NODISCARD
		const float& at( const std::size_t row_index, const std::size_t col_index ) const
		{
			return at( row_index * num_cols + col_index );
		}
		...
		NODISCARD
		bool all_of( const float value ) const
		{
			return std::all_of( _data.begin( ), _data.end( ), [ &value ]( const float x ) { return x == value; } );
		}
		...
		matrix_t< 1, num_cols > row( const std::size_t index )
		{
		...
		matrix_t< num_rows, 1 > col( const std::size_t index )
		{
		...
		void row( const std::size_t index, const matrix_t< 1, num_cols >& row_values )
		{
			for ( std::size_t i = 0; i < num_cols; ++i ) {
		...
		void col( const std::size_t index, const matrix_t< num_rows, 1 >& col_values )
		{
			for ( std::size_t i = 0; i < num_rows; ++i ) {
		...
		void set( const type_array& values )
		{
			_data = values;
		}
		...
		NODISCARD float dot( const matrix_t< rhs_num_rows, rhs_num_cols >& rhs ) const
		{
		...
		NODISCARD
		matrix_t cross( const matrix_t& rhs ) const
		{
		...
		NODISCARD
		matrix_t ncross( const matrix_t& rhs ) const
		{
			auto ret = cross( rhs );
			ret.normalize( );
			return ret;
