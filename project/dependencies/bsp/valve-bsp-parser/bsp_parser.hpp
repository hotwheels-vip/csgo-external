...
namespace rn
{
	...
	public:
		...
		bsp_parser( const bsp_parser& rhs ) = delete;

		bsp_parser& operator=( const bsp_parser& rhs ) = delete;

		bsp_parser( bsp_parser&& rhs ) noexcept;

		bsp_parser& operator=( bsp_parser&& rhs ) noexcept;

	private:
		bool set_current_map( const std::string& directory, const std::string& map_name, std::string& file_path );

		bool parse_planes( std::ifstream& file, std::optional< valve::lumpfileheader_t > lumpFileHeader );
		bool parse_entities( std::ifstream& file, std::optional< valve::lumpfileheader_t > lumpFileHeader );

		bool parse_nodes( std::ifstream& file, std::optional< valve::lumpfileheader_t > lumpFileHeader );

		bool parse_leaffaces( std::ifstream& file, std::optional< valve::lumpfileheader_t > lumpFileHeader );

		bool parse_leafbrushes( std::ifstream& file, std::optional< valve::lumpfileheader_t > lumpFileHeader );
		...
		void ray_cast_node( std::int32_t node_index, float start_fraction, float end_fraction, const vector3& origin, const vector3& destination,
		                    valve::trace_t* out );

		void ray_cast_surface( std::int32_t surface_index, const vector3& origin, const vector3& destination, valve::trace_t* out );

		void ray_cast_brush( valve::dbrush_t* brush, const vector3& origin, const vector3& destination, valve::trace_t* out ) const;

		template< typename type >
		NODISCARD bool parse_lump( std::ifstream& file, const valve::lump_index lump_index, std::vector< type >& out,
		                           std::optional< valve::lumpfileheader_t > fileLump = std::nullopt ) const
		{
			...
			if ( has_valid_lzma_ident( lzma_header.id ) ) {
				assert( lump_index != valve::lump_index::game_lump ||
				        lump_index != valve::lump_index::pak_file ); // Those have special rules regarding compression.
				...
				LzmaUncompress( reinterpret_cast< unsigned char* >( tmpUncompressedData ), &realSize,
				                reinterpret_cast< unsigned char* >( tmpData + sizeof( lzma_header_t ) ), // point to actual data, not the header
				                &lzmaSize, reinterpret_cast< unsigned char* >( lzma_header.properties.data( ) ), LZMA_PROPS_SIZE );
	...
	public:
		bool load_map( const std::string& directory, const std::string& map_name );

		bool is_visible( const vector3& origin, const vector3& destination );

		void trace_ray( const vector3& origin, const vector3& final, valve::trace_t* out );
		void unload_map( );
